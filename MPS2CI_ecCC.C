...
//    }
    ...
    void evaluate(const std::shared_ptr<simplemps> &mps, const std::vector<int> reorder, int nocc, int nvir, bool noreorder, double cutoff = 1e-10, int max_ex = 4) {
        ...
        //210625
        int Ms = dmrginp.molecule_quantum().get_s().getirrep();  //This is 2*Sz of left quanta num.
...
//        //lsh test
//        std::cout << "nsite: " << n_sites << ", max_ex: " << max_ex << std::endl;
...
void MPS2CI_run(double cutoff, int max_ex)
{
  ...
  // Ref append
  bool noreorder = true;
  std::vector<uint8_t> Refdet(nocc+nvir, 0), tmpdet;
  ...
  dtrie.evaluate(zeromps, reorder, nocc, nvir, noreorder, cutoff, max_ex);
