...
class simplemps
{
  public:
    ...
    virtual void lw_dot_mat(const int d, const int j, SQ_RV &lw, SQ_RV &nw)=0;
    virtual void lw_dot_mat_new(const int d, const int j, partial_wv &lw, partial_wv &nw)=0;
...
class Abelianmps :public simplemps
{
  ...
  public:
    ...
    void lw_dot_mat(const int d, const int j, SQ_RV &lw, SQ_RV &nw)=0;
    void lw_dot_mat_new(const int d, const int j, partial_wv &lw, partial_wv &nw)=0;
    void initstate();
...
class NonAbelianmps :public simplemps
{
  ...
  public:
    ...
    double getcoeff(const bitstring& ci);
    double sampling(bitstring& ci);
    void lw_dot_mat(const int d, const int j, SQ_RV &lw, SQ_RV &nw);
    void lw_dot_mat_new(const int d, const int j, partial_wv &lw, partial_wv &nw);
    void initstate();
    const double get_norm() const {return norm;}
    void print_timer(){
      cout <<"coeff time: "<<mpscoeffT<<endl;
