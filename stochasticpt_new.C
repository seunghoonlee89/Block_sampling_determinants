...
*/
  ...
  std::unordered_map<bitstring, int> det_histo;
  ...
  //sd_hashtable.reserve(num_sample);
  {
    ...
    for (int i=0;i<num_sample;i++)
    {
      ...
      if(det_histo.find(determinant)==det_histo.end()) // check whether the sampled determinant has already been sampled before.
      {
        det_histo[determinant] = 1;
      }
      else{
        det_histo[determinant] += 1;
      }
...
// gen histo only with one node
  {
  const std::vector<int> ref_occ = dmrginp.hf_occupancy();

  int tot=0;
  for (auto i : det_histo)
      tot += i.second; 
  ...
  for (auto i : det_histo)
  {
      ...
      int ptcl = 0;
      int excite = 0;
      for (int itmp=0; itmp<ref_occ.size(); itmp++) {
          if (ref_occ.at(itmp) == 1) {
              if ( i.first.test(itmp) == 0 )
                  excite += 1;
              nhole += 1;
              hole  += int(i.first.test(itmp)) * pow(2, nhole-1);
          }
          else{
              nptcl += 1;
              ptcl  += int(i.first.test(itmp)) * pow(2, nptcl-1);
          }
      }
      pout << excite << "th excitation ";

      int test = 0;
      for (int j=0; j<i.first.size(); j++)
      {
          test += int(i.first.test(j)) * pow(2, j);
          pout << i.first.test(j);
      } 

      pout << " " << hole << " " << ptcl << " " << test;
      pout << " " << double(i.second)/double(tot) <<endl;
  }
