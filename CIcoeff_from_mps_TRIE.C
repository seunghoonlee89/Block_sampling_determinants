...
struct DeterminantTRIE {
    ...
    // deep copy
    std::shared_ptr<DeterminantTRIE> copy() {
        std::shared_ptr<DeterminantTRIE> dett =
            std::make_shared<DeterminantTRIE>(n_sites, enable_look_up);
        dett->data = std::vector<std::array<size_t, 4>>(data.begin(), data.end());
        dett->dets = std::vector<size_t>(dets.begin(), dets.end());
        dett->invs = std::vector<size_t>(invs.begin(), invs.end());
...
//             << unsigned(j) << " " << cur << " data[cur][j] = " << data[cur][j] << std::endl;
    ...
    void evaluate(const std::shared_ptr<simplemps> &mps, double cutoff = 0) {
        vals.resize(dets.size());
        std::memset(&vals[0], 0, sizeof(double) * vals.size());
        std::stack<std::tuple<size_t, int, int>> ptrs;
        for (uint8_t j = 0; j < (int)data[0].size(); j++)
            if (data[0][j] != 0)
                ptrs.push(std::make_tuple(data[0][j], j, 0));
        std::vector<SQ_RV> partials;
        RowVector oldwave(1);
        oldwave(1) = 1.0;
        //TODO: lsh
        ...
        // with LLLL .. LC canonical form
        // lwave: 
        //          |
        //      0 --*-- 
        //(lwave[0]: target quanta)
        SQ_RV lwave;
        lwave[std::make_tuple(0, 0, 0, 0)] = oldwave;
        int Ms = 0;//This is 2*Sz of left quanta num.
        partials.push_back(lwave);
...
//            //lsh test
//            std::cout << std::endl;
//            std::cout << "cur, j, d = " << cur << " " << j << " " << d << std::endl;
            //   j
            //   |
            // --*--
            //  d+1 
            ...
            partials.resize(d + 1);
            partials.push_back(SQ_RV());
            SQ_RV &lw = partials[d], &nw = partials[d+1];
...
//              SQ SQl = iter->first;
            ...
            //         d              d+1                             d+1
            //input:  d+1, dotq, leftq, Ms, dotMs,
            //output: nw<SQ,RowVector>
            mps->lw_dot_mat(d, j, lw, nw);
...
//            }
            ...
            if (d == n_sites - 1) {
                ...
                for (uint8_t jj = 0; jj < (int)data[cur].size(); jj++)
                    if (data[cur][jj] != 0)
                        ptrs.push(std::make_tuple(data[cur][jj], jj, d + 1));
            }
        }
    }
};
...
void CIcoeff_from_mps_TRIE(double thresh)
{
...
////============================== test ========================================  
//  //lsh test: ref
//  DeterminantTRIE dtrie(nsite, true); 
...
//    cout <<string(i)<<endl;
  ...
  double thresh = dmrginp.stochasticpt_tol();

  CIcoeff_from_mps_TRIE(thresh);
