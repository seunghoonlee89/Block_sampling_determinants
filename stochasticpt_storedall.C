...
void heatbath_pt(const unsigned long num_sample)
{
  StateQuantaInfo baseState(0);
  baseState.preparestate();
  ...
  for(auto iter = sd_hashtable.begin();iter!=sd_hashtable.end();iter++)
  {
    ...
    determinant1[0] = iter->first.to_vector(n_spinorb);
    std::vector<double> coeff1(3);
    coeff1[0] = iter->second;
    double coeff = coeff1[0];
    double overlap= baseState.local_energy(determinant1[0], 1);
    if(sd_hashtable1.find(iter->first)==sd_hashtable1.end())
    {
      sd_hashtable1[iter->first] = coeff*overlap;
    }
    else
    {
      sd_hashtable1[iter->first] += coeff*overlap;
    ...
    {
      baseheatbath.doubleexcite(determinant1[0], coeff1[0]/n_H_samples, determinant1[2], coeff1[2]);
      baseheatbath.singleexcite(determinant1[0], coeff1[0]/n_H_samples, determinant1[1], coeff1[1]);
      longbitarray cistring;
      cistring.from_vector(determinant1[1]);
      if(sd_hashtable1.find(cistring)==sd_hashtable1.end())
      {
        sd_hashtable1[cistring] = coeff1[1];
      }
      else
      {
        sd_hashtable1[cistring] += coeff1[1];
      }
      cistring.from_vector(determinant1[2]);
      if(sd_hashtable1.find(cistring)==sd_hashtable1.end())
      {
        sd_hashtable1[cistring] = coeff1[2];
      }
      else
      {
        sd_hashtable1[cistring] += coeff1[2];
  ...
  pout <<"Size of QV|0>" << sd_hashtable1.size()<<endl;

  std::unordered_map<longbitarray, double> sd_hashtable2;
  for(auto iter = sd_hashtable1.begin();iter!=sd_hashtable1.end();iter++)
  {
    ...
    std::vector<double> coeff1(3);
    coeff1[0] = iter->second/local_energy0;
    double coeff = coeff1[0];
    double overlap= baseState.local_energy(determinant1[0], 1);
    if(sd_hashtable2.find(iter->first)==sd_hashtable2.end())
    {
      sd_hashtable2[iter->first] = coeff*overlap;
    }
    else
    {
      sd_hashtable2[iter->first] += coeff*overlap;
    ...
    for(int i=0;i<n_H_samples;i++)
    {
      ...
      longbitarray cistring;
      cistring.from_vector(determinant1[1]);
      if(sd_hashtable2.find(cistring)==sd_hashtable2.end())
      {
        sd_hashtable2[cistring] = coeff1[1];
      }
      else
      {
        sd_hashtable2[cistring] += coeff1[1];
      ...
      if(sd_hashtable2.find(cistring)==sd_hashtable2.end())
      {
        sd_hashtable2[cistring] = coeff1[2];
      }
      else
      {
        sd_hashtable2[cistring] += coeff1[2];
  ...
  pout <<"Size of VQ(H0-E0)-1QV|0>" << sd_hashtable2.size()<<endl;

  for(auto iter = sd_hashtable2.begin();iter!=sd_hashtable2.end();iter++)
  {
    std::vector<int> determinant = iter->first.to_vector(n_spinorb);
    double overlap= baseState.getcoeff(determinant);
    H11 += overlap*iter->second;
    H11_2 += overlap*iter->second*overlap*iter->second;
